...
router.post('/sendMails/:topicId', async (req, res) => {
   try {
    ...
    const emailsSent = await Promise.all(topic.author_works.flatMap(async (author_work) => {
        // const acceptLinks = author_work.reviewers.map(reviewer => {
        //     return `http://localhost:3000/review-paper2?reviewerId=${reviewer._id}&authorWorkId=${author_work._id}`;
        // });
